    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            if i % 1000 == 0:
                ...
                print("After %d trainning steps, validation accuracy using average model is %g" % (i, validate_acc))
            xs, ys = mnist.train.next_batch(BATCH_SIZE)
            sess.run(train_op, feed_dict={x: xs, y_: ys})

        test_acc = sess.run(accuracy, feed_dict=test_feed)
        print("After %d trainning steps, test accuracy using average model is %g" % (TRAINING_STEPS, test_acc))
